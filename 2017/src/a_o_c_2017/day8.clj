...
(defn- swap-val [reg r f val]
  (assoc reg (f (get-val reg r) val)))

(defn eval-predicate [reg r p val]
  (let [p (if (=  p "!=") "not=" p)
        r (get-val reg r)
        s (str "(" p " " r " " val ")")]
        ;_ (prn s)]
...
(defn eval-exp [reg r f val]
  (let [r-val (get-val reg r)
        val (read-string val)
        f (get fn-map f)]
    (assoc reg r (f r-val val))))

(defn eval-line [reg [exp-r exp-fn exp-val _ pred-r pred-p pred-val]]
  (if (eval-predicate reg pred-r pred-p pred-val)
    (eval-exp reg exp-r exp-fn exp-val)
    reg))
...
(def data 
  (->> raw-data
       ...
       (map #(str/split % #" ")))) 
                                        ...
                                        ;3612 is the correct answer

(defn part-1 [reg data]
  (->> (reduce eval-line reg data)
       (vals)
       (apply max)))

(defn eval-line-max [reg [exp-r exp-fn exp-val _ pred-r pred-p pred-val]]
  (if (eval-predicate reg pred-r pred-p pred-val)
    (let [reg (eval-exp reg exp-r exp-fn exp-val)
          curr-max (get reg :max (- Integer/MAX_VALUE))
          curr-update (get reg exp-r)]
      (assoc reg :max (max curr-max curr-update)))
    reg))

                                        ;3818 is the correct answer
(defn part-2 [reg data]
  (->> (reduce eval-line-max reg data)
       (:max)))



(def raw-data "v inc 523 if t == 6
