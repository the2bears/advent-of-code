...
(defn valid? [line]
  (let [words (str/split line #" ")
        word-set (set words)]
    (= (count words) (count word-set))))

                                        ;337 is the correct answer
...
(defn word->map [word]
  (let [ch-seq (seq word)]
    (loop [s ch-seq
      ...
      (if-not (seq s)
        acc
        (recur (rest s) (assoc acc
                               (first s)
                               (inc (get acc (first s) 0))))))))     

(defn anagrams? [line]
  (let [words (str/split line #" ")
        word-maps (set (map word->map words))]
    (not= (count word-maps) (count words))))
...
(def data "kvvfl kvvfl olud wjqsqa olud frc
